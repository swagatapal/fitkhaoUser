...
class _OrderTrackingScreenState extends ConsumerState<OrderTrackingScreen> {
  ...
  bool get _shouldShowCancelButton {
    ...
    try {
      ...
      if (deliveryDay == dayAfterTomorrow &&
          (now.isBefore(tomorrow.add(const Duration(days: 1))))) {
        return true;
      }
      ...
      debugPrint(
          '[OrderTrackingScreen] Error checking cancel button visibility: $e');
      return false;
    }
  }
      ...
      child: Column(
        ...
        children: [
          ...
          _buildDetailRow(Icons.access_time, 'Delivery Slot',
              _formatDeliverySlot(widget.order.deliverySlot)),
          const SizedBox(height: AppSizes.spacing8),
          _buildDetailRow(Icons.phone, 'Contact',
              widget.order.deliveryAddress.contactNumber),
  ...
  Widget _buildPriceSummary() {
    ...
    final deliveryCharge = widget.order.deliveryCharge;
    final discount = widget.order.discount;
    final total = widget.order.total;

    final knownTotal = subtotal + tax + deliveryCharge - discount;
    final otherCharges = total - knownTotal;
    final showOtherCharges = otherCharges.abs() > 0.009;
      ...
      child: Column(
        ...
        children: [
          ...
          if (showOtherCharges) ...[
            const SizedBox(height: AppSizes.spacing8),
            _buildPriceRow(
              otherCharges >= 0 ? 'Platform Charge' : 'Adjustment',
              otherCharges.abs(),
              isDiscount: otherCharges < 0,
            ),
          ],
          const SizedBox(height: AppSizes.spacing8),
        ...
        children: [
          ...
          ...List.generate(steps.length, (index) {
            ...
            final currentStatusIndex =
                _getStatusIndex(widget.order.orderStatus);
            final stepIndex = _getStatusIndex(stepStatus);
            final isCompleted = currentStatusIndex > stepIndex;
            final isCurrent = widget.order.orderStatus.toLowerCase() ==
      ...
      children: [
        Expanded(
          child: OutlinedButton.icon(
            ...
            icon:
                const Icon(Icons.support_agent, color: AppColors.primaryGreen),
            label: const Text(
              'Help & Support',
              style: TextStyle(
                fontSize: AppTypography.fontSize14,
  ...
  String _formatDeliverySlot(DeliverySlotInfo slot) {
    final name = slot.slotName.trim();
    final timeRange = slot.timeRange.trim();

    if (name.isEmpty && timeRange.isEmpty) return '-';
    if (name.isNotEmpty && timeRange.isNotEmpty) return '$name ($timeRange)';
